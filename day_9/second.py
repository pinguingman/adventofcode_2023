...
def get_next_value(values):
    ...
    for i in range(len(differences) - 2, -1, -1):
        # we don't need to actually store a new values in the beginning
        # as specified in the task because we only need the result
        differences[i].append(differences[i][0] - differences[i + 1][-1])
